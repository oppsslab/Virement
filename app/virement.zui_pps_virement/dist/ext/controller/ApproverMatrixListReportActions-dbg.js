sap.ui.define(
  ...
  function (MessageBox, MessageToast, Fragment, ResourceModel) {
    ...
    /*
     * The Approver Matrix List Report has no parent record to bind
     * to - downloadApproverMatrixTemplate and uploadApproverMatrix
     * are both unbound service actions, and created rows go straight
     * into the top-level /ApproverMatrix collection. This mirrors
     * ObjectPageCustomActions.js's file-handling approach, but does
     * not need its binding-context resolution: there is no context.
     */
    ...
    function triggerDownload(sBase64, sFileName, sMimeType) {
      ...
      oLink.href = sUrl;
      oLink.download = sFileName || "Approver Matrix Template.xlsx";
    ...
    function handleConfirmUpload() {
      ...
      oOperation
        ...
        .then(function () {
          /*
           * Rows were created via a separate list binding, so force
           * every active binding on this model - including the List
           * Report's own table - to re-fetch.
           */
          ...
          MessageToast.show("Approver Matrix rows uploaded successfully.");
        })
    ...
    return {
      onDownloadTemplate: function (oEvent) {
        ...
        const oOperation = oModel.bindContext(
          "/downloadApproverMatrixTemplate(...)",
        );
      ...
      onOpenUploadDialog: function (oEvent) {
        ...
        if (!oUploadDialog) {
          Fragment.load({
            id: oView ? oView.getId() : "approverMatrixUploadFrag",
            name: "virement.zuippsvirement.ext.fragment.ApproverMatrixUploadDialog",
            controller: this,
          })
