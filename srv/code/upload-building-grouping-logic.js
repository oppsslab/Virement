...
const { TEMPLATE_COLUMNS } = require("./utils/building-grouping-template");

const LOG = cds.log("upload-building-grouping-logic");
...
function validateRow(row) {
  ...
  if (!state) {
    errors.push({
      row: rowNo,
      field: "state",
      message: `Row ${rowNo}: State is required.`,
    });
  }
...
module.exports = async function uploadBuildingGrouping(request) {
  LOG.info("--- uploadBuildingGrouping started ---");
  ...
  try {
    ...
    LOG.error("uploadBuildingGrouping failed unexpectedly:", error);
