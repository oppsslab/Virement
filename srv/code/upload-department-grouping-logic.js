...
const { TEMPLATE_COLUMNS } = require("./utils/department-grouping-template");

const LOG = cds.log("upload-department-grouping-logic");
...
function validateRow(row) {
  ...
  if (!department) {
    errors.push({
      row: rowNo,
      field: "department",
      message: `Row ${rowNo}: Department is required.`,
    });
  }
...
module.exports = async function uploadDepartmentGrouping(request) {
  LOG.info("--- uploadDepartmentGrouping started ---");
  ...
  try {
    ...
    LOG.error("uploadDepartmentGrouping failed unexpectedly:", error);
