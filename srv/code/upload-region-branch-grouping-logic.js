...
const { TEMPLATE_COLUMNS } = require("./utils/region-branch-grouping-template");

const LOG = cds.log("upload-region-branch-grouping-logic");
...
function validateRow(row) {
  ...
  const region = toTrimmedString(row.region);
  ...
  const costCentreDescription = toTrimmedString(row.costCentreDescription);

  if (!region) {
    errors.push({
      row: rowNo,
      field: "region",
      message: `Row ${rowNo}: Region is required.`,
    });
  }
  ...
  if (!branch) {
    errors.push({
      row: rowNo,
      field: "branch",
      message: `Row ${rowNo}: Branch is required.`,
    });
  }
  ...
  if (!costCentre) {
    errors.push({
      row: rowNo,
      field: "costCentre",
      message: `Row ${rowNo}: Cost Centre is required.`,
    });
  }
...
module.exports = async function uploadRegionBranchGrouping(request) {
  LOG.info("--- uploadRegionBranchGrouping started ---");
  ...
  try {
    ...
    LOG.error("uploadRegionBranchGrouping failed unexpectedly:", error);
